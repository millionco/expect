...
const hasHelium = fs.existsSync(HELIUM_PROFILE_PATH) && fs.existsSync(HELIUM_EXECUTABLE_PATH);

describe.skipIf(!hasHelium)("Helium CDP", () => {
  it.live(
    "extracts cookies from Helium via CDP",
    () =>
      Effect.gen(function* () {
        ...
        if (cookies.length > 0) {
          console.error(
            `[Helium] first 5:`,
            cookies.slice(0, 5).map((c) => `${c.name}@${c.domain}`),
          );
        }
  ...
  it.live(
    "Helium profile directory contains expected files",
    () =>
      Effect.gen(function* () {

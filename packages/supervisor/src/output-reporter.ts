...
import { EXPECT_STATE_DIR, ARTIFACTS_DIRECTORY_NAME } from "./constants";
...
>()("@supervisor/OutputReporterHooks") {
  ...
  static layerGitHubActions = Layer.effect(this)(
    Effect.gen(function* () {
      ...
      const replayPath = `${repoRoot}/${EXPECT_STATE_DIR}/${ARTIFACTS_DIRECTORY_NAME}/${planId}.ndjson`;
    ...
    Layer.effect(OutputReporter)(
      Effect.gen(function* () {
        ...
        const write = (text: string) =>
          Stream.make(text).pipe(Stream.run(stdio.stdout())).pipe(Effect.orDie);
  ...
  static layerJson = Layer.effect(OutputReporter)(
    Effect.gen(function* () {
      ...
      return {
        ...
        onComplete: (report: TestReport) =>
          Effect.gen(function* () {
            const encoded = yield* Schema.encodeEffect(TestReport.json)(
              new TestReport({
                ...report,
                diffPreview: "",
                events: [],
...
const printSummary = (write: (text: string) => Effect.Effect<void>, report: TestReport) =>
  Effect.gen(function* () {
    yield* write("");
    const parts: string[] = [];

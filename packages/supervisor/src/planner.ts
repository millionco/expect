...
export class PlanParseError extends Schema.ErrorClass<PlanParseError>(
  "@supervisor/PlanParseError"
)({
...
export class PlanningError extends Schema.ErrorClass<PlanningError>(
  "@supervisor/PlanningError"
)({
  ...
  Effect.gen(function* () {
    const raw = yield* Effect.try({
      try: () => fs.readFileSync(sentinelPath, "utf-8"),
        ...
        new PlanParseError({ cause: `Could not read plan file: ${cause}` }),
    });

    const parsed = yield* Schema.decodeEffect(
      Schema.fromJsonString(TestPlanJson)
    )(raw).pipe(
      Effect.mapError(
        (cause) => new PlanParseError({ cause: `Invalid plan JSON: ${cause}` })
      )
    );
      ...
      (step, index) =>
        new TestPlanStep({
          id: Schema.decodeSync(StepId)(
            step.id ??
              `step-${String(index + 1).padStart(STEP_ID_PAD_LENGTH, "0")}`
          ),
          title: step.title,
          instruction: step.instruction,
          expectedOutcome: step.expectedOutcome,
          routeHint: step.routeHint
            ? Option.some(step.routeHint)
            : Option.none(),
          status: "pending",
          summary: Option.none(),
        })
    ...
    return new TestPlan({
      ...draft,
      id: Schema.decodeSync(PlanId)(
        parsed.id ?? `plan-${crypto.randomUUID().slice(0, 8)}`
      ),
      title: parsed.title,
...
export class Planner extends ServiceMap.Service<Planner>()(
  "@supervisor/Planner",
  {

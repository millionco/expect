import { Cookies, layerLive } from "@expect/cookies";
...
export interface CreateSessionOptions {
  headless: boolean;
  browserOverride?: "chromium" | "webkit" | "firefox";
  ...
  make: Effect.gen(function* () {
    ...
    const withCreateSession =
      ...
      > =>
        Effect.gen(function* () {
          ...
          const engine = browserOverride ?? "chromium";
          const browser = yield* Effect.acquireRelease(
            Effect.tryPromise({
              try: () => {
                if (cdpUrl._tag === "Some") return chromium.connectOverCDP(cdpUrl.value);
                if (engine === "webkit") return webkit.launch({ headless });
                if (engine === "firefox") return firefox.launch({ headless });
    ...
    const runSession = Effect.fn("Playwright.runSession")(
      function* (options: CreateSessionOptions) {
        ...
        yield* Effect.addFinalizer(() => Effect.logDebug(`Browser being cleaned up`));
        ...
        const pollOnce = Effect.gen(function* () {
          if (page.isClosed())
            return yield* new PlaywrightExecutionError({
              cause: "Page is closed!",
            });
          const recording = yield* evaluateRuntime(page, "isRecording");
          if (!recording) {
            yield* Effect.logInfo("rrweb recording lost after navigation, restarting");
            yield* evaluateRuntime(page, "startRecording");
          }
          const events = yield* evaluateRuntime(page, "getEvents");

          /**
           * @note(rasmus): we do this because we're too lazy to properly make schemas for Rrweb events, so
           * undefined values don't get properly serialized when they cross the RPC boundary
           */
          return events.map(
            (event) => new RrwebEvent({ event: JSON.parse(JSON.stringify(event)) }),
          );
        }).pipe(
          Effect.tapCause((cause) => Effect.logError(`Gathering Rrweb events failed`, cause)),
          Effect.catch(() => Effect.succeed([] as RrwebEvent[])),
        );
        ...
        // yield* Stream.merge(rrwebEvents, pageEvents).pipe(
        yield* rrwebEvents.pipe(
          Stream.chunks,
    ...
    const open = Effect.fn("Playwright.open")(function* (options: CreateSessionOptions) {
      ...
      return yield* Effect.suspend(() =>
        ...
        Effect.catchTag("TimeoutError", () =>
          FiberHandle.clear(handle).pipe(
            Effect.flatMap(() => Effect.fail(new BrowserNotOpenError())),
          ),
        ),
      );
    });
    ...
    const execute = Effect.fn("Playwright.execute")(function* (
      ...
      return yield* Effect.tryPromise({
        try: async () => {
          ...
          return await userFunction(page, page.context(), page.context().browser(), ref);
        },

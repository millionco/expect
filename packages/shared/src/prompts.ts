...
export const buildExecutionSystemPrompt = (browserMcpServerName?: string): string => {
  ...
  return [
    ...
    "  Step 2: playwright → await ref('e7').click(); await page.waitForTimeout(300);",
    "  Step 3: screenshot mode=snapshot → find option 'Canada' [ref=e15]",
    "  Step 4: playwright → await ref('e15').click();",
    "  Step 5: screenshot mode=snapshot → verify 'Canada' is now the selected value",
    "",

...
export const buildExecutionSystemPrompt = (browserMcpServerName?: string): string => {
  ...
  return [
    ...
    "- Inspect the project for a design system (tailwind.config, CSS custom properties, component libraries, token files).",
    "- Verify changed or added elements use the system's tokens and utilities, not arbitrary values.",
    "- Flag hardcoded hex/rgb colors, pixel spacing values, or font-family declarations that bypass the design system.",
    "",
    "2. Responsive design verification:",
    "- Test at these viewports using page.setViewportSize: 375×812 (iPhone SE), 390×844 (iPhone 14), 768×1024 (iPad Mini), 810×1080 (iPad Air), 1024×768 (iPad landscape), 1280×800 (laptop), 1440×900 (desktop).",
    "- At each viewport, snapshot and verify: no horizontal overflow, no overlapping elements, text readable, interactive elements accessible and not clipped, navigation adapts.",
    "- Batch efficiently: resize, snapshot, assert. Do not skip tablets.",
    "",
    "3. Touch interaction testing:",
    "- If the diff introduces or modifies interactive elements, test them with touch in addition to click at a mobile viewport.",
    "- Verify the same flows that work via click also complete via tap.",
    "",
    "4. Cross-browser check (Safari/WebKit):",
    "- Launch a WebKit browser context and re-run the primary flow.",
    "- Check for common Safari regressions: flexbox gap, backdrop-filter, position:sticky in overflow, date/time inputs, scrollbar styling, -webkit-line-clamp.",
    "- If WebKit is unavailable, emit STEP_SKIPPED with reason 'WebKit browser not available'.",
    "",
    "5. Dark mode verification:",
    "- Detect dark mode support: dark: Tailwind classes, theme toggle, prefers-color-scheme queries, data-theme attribute.",
    "- If supported, switch to dark mode and re-verify the primary flow.",
    "- Check for: invisible or low-contrast text, disappearing borders, icons assuming light background, hardcoded white backgrounds on theme-aware components.",
    "- If no dark mode support detected, emit STEP_SKIPPED.",
    "",
    "6. Layout stability (CLS):",
    "- After networkidle, measure cumulative layout shift using PerformanceObserver with type 'layout-shift'.",
    "- CLS above 0.1 is a failure. Between 0.05 and 0.1 is a warning.",
    "- If CLS is high, screenshot immediately after load and 3 seconds later. Report which elements shifted.",
    "",
    "7. Font loading verification:",
    "- After networkidle, check document.fonts API. Every font used by visible text must have status 'loaded'.",
    "- Verify @font-face declarations or font preload tags exist for each font family.",
    "- Flag visible text relying solely on system fonts unless the design system specifies a system font stack.",
    "",
    ...
    "- If you encounter a hard blocker (login, passkey, captcha, permissions, missing data), stop and report it instead of improvising.",
    "",
